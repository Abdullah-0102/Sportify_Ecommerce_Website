...
<section class="row">
  ...
  <div class="col-md-5 col-sm-12" style="padding:40px;">
    ...
    <div class="row quantity">
      ...
      <div class="col-lg-8">
        <a class="add-to-cart" style="color: white;">ADD TO CART</a>
      </div>
    </div>
...
@php
    $productAttributes = [
        ...
        'img1' => $p->img1, // Assuming this is the attribute you want to access
        // Add more attributes as needed
    ];
    $product = json_encode($productAttributes);
@endphp
@endforeach
...
<script>
  document.addEventListener("DOMContentLoaded", () => {
    document.querySelector(".add-to-cart").addEventListener('click', () => {
      //ensure that a size and color is selected.
      const clr = document.querySelector('.color-option:checked').value;
      

      const size = document.querySelector(".size-label");
      // Assuming checkedSizeRadioButton.innerHTML contains "Size: M"
      var text = size.innerHTML;

      // Find the position of the colon
      var colonIndex = text.indexOf(':');

      // Extract the part after the colon
      var partAfterColon = text.substring(colonIndex + 1).trim(); // Get the part after the colon and remove extra spaces
      // Output: "M" (part after the colon)
      if(partAfterColon == "")
        ...
        // let encodedItem = encodeURIComponent(JSON.stringify(cartItem));
        let url = `/add-to-cart/${cartItem.id}/${cartItem.quantity}/${cartItem.color}/${cartItem.size}`;
